...
export default function UserTable({ data, onUpdate, onDelete, onUpdateUserBalance }: Props) {
      ...
      {data.map((item, index) => {
        ...
        return (
          <Fragment key={index}>
            ...
            <StyledBodyCell $striped={striped}> {(item.fullname || '').slice(0, 10)}</StyledBodyCell>
